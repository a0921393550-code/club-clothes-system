...
# 基本設定
# =========================
SPREADSHEET_URL = "https://docs.google.com/spreadsheets/d/1rwiSSLJQaoBTH8Std8lBW03deOJ9RpksA6rhxWiqmH8/edit?gid=0#gid=0"
...
with tab1:
    ...
    with st.form("clothes_form"):
        ...
        if submitted:
            ...
            clean_clothes = clothes.strip()

            if not clean_name or not clean_clothes:
    ...
    if not calc.empty:
        calc["姓名"] = calc["姓名"].astype(str).apply(normalize_text)
        calc["服裝名稱"] = calc["服裝名稱"].astype(str).str.strip()
...
with tab2:
    ...
    with st.form("fee_form"):
        ...
        if fee_submitted:
            clean_member_name = normalize_text(member_name)
            clean_fee_type = fee_type.strip()

            if not clean_member_name:
                st.warning("⚠️ 請輸入姓名")
            elif not clean_fee_type:
    ...
    if not members_df.empty:
        ...
        if not fee_calc.empty:
            fee_calc["姓名"] = fee_calc["姓名"].astype(str).apply(normalize_text)
...
with tab3:
    ...
    if admin_pw == ADMIN_PASSWORD:
        ...
        with st.form("member_form"):
            ...
            if member_submitted:
                clean_name = normalize_text(new_member_name)
